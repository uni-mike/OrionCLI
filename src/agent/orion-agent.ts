import { AzureClient as OrionClient, AzureMessage as OrionMessage, AzureToolCall as OrionToolCall } from "../azure/client";
import { ToolDefinition, ToolCall } from "../orion-compat/tools";
...
export class OrionAgent extends EventEmitter {
  private grokClient: OrionClient;
  ...
  constructor(
    ...
    this.grokClient = new OrionClient(apiKey, modelToUse, baseURL);
...
Current working directory: ${process.cwd()}`,
  ...
  private async executeTool(toolCall: OrionToolCall): Promise<ToolResult> {
    try {
  ...
  private async executeMCPTool(toolCall: OrionToolCall): Promise<ToolResult> {
    try {
      const args = JSON.parse(toolCall.function.arguments);
